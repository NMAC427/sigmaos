...
std::expected<std::shared_ptr<std::string>, sigmaos::serr::Error> Clnt::GetFile(
  ...
  if (rep.blob().iov_size() > 0) {
    return std::make_shared<std::string>(rep.blob().iov(0));
  } else {
